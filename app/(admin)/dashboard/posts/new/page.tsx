...
export default function NewPostPage() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name === "title") {
      const generatedSlug = value.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, ''); 
      setFormData(prev => ({ ...prev, title: value, slug: generatedSlug }));
    } else if (name === "slug") {
      const safeSlug = value.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9-]+/g, '').replace(/--+/g, '-');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.slug) { appleAlert.fire({ icon: 'warning', title: 'URL Incompleta', text: 'El slug es obligatorio.' }); return; }
    ...
    if (formData.tags.length === 0) { appleAlert.fire({ icon: 'warning', title: 'Sin Categorías', text: 'Selecciona al menos una etiqueta.' }); return; }
    ...
    try {
      ...
      await appleAlert.fire({ icon: 'success', title: '¡Publicado!', text: 'Tu nuevo artículo ya está en línea.', confirmButtonText: 'Continuar' });
      router.push("/dashboard/posts"); 
      router.refresh();
    } catch (error: any) {
      appleAlert.fire({ icon: 'error', title: 'Error de Servidor', text: 'No se pudo crear el artículo.' });
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-6xl mx-auto pb-24 px-4 sm:px-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
        ...
        <div className="flex items-center">
            <button onClick={handleSubmit} disabled={loading || uploadingImage} className="px-6 py-2.5 w-full sm:w-auto bg-black hover:bg-gray-800 text-white rounded-xl shadow-md transition-all font-medium text-sm flex items-center justify-center gap-2 disabled:opacity-50">
                {loading ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />} Publicar Artículo
            </button>
        </div>
      </div>
        ...
        <div className="lg:col-span-8 space-y-6">
            <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] space-y-8">
                <div>
                    ...
                    <input type="text" name="title" placeholder="Escribe un título llamativo..." className="w-full text-3xl font-bold text-gray-900 placeholder:text-gray-300 border-none focus:ring-0 p-0 bg-transparent" value={formData.title} onChange={handleChange} required />
                </div>
                <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                    ...
                    <input type="text" name="slug" className="w-full text-sm font-mono text-blue-600 border-none focus:ring-0 p-0 bg-transparent" value={formData.slug} onChange={handleChange} required />
                </div>
                <div>
                    <label className="block text-[11px] font-semibold text-gray-400 uppercase tracking-widest mb-3">Resumen para la tarjeta</label>
                    <textarea name="excerpt" rows={3} placeholder="Una breve introducción de 2 o 3 líneas..." className="w-full text-gray-600 placeholder:text-gray-300 border-none focus:ring-0 p-0 bg-transparent resize-none leading-relaxed" value={formData.excerpt} onChange={handleChange} required />
                </div>
            </div>
        ...
        <div className="lg:col-span-4 space-y-6">
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
                <div onClick={() => setFormData(prev => ({ ...prev, isFeatured: !prev.isFeatured }))} className="flex items-center justify-between cursor-pointer group">
                    <div className="flex items-center gap-3">
                        ...
                        <div><p className="text-sm font-bold text-gray-900">Destacar Artículo</p><p className="text-[11px] text-gray-400 mt-0.5">Fijar al inicio del blog</p></div>
                    </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.02)] space-y-8">
                ...
                <div>
                    ...
                    <input type="text" name="readTime" placeholder="Ej: 5 min" className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all" value={formData.readTime} onChange={handleChange} required />
                </div>
            </div>
        </div>
      </form>
