...
import { LayoutDashboard, FileText, LogOut, Menu, X, PlusCircle, Home, ShieldCheck } from "lucide-react";
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // Datos de usuario estáticos para la UI
  const mockUser = {
    name: "Administrador",
  ...
  const handleSignOut = () => {
    console.log("Cerrando sesión (Frontend only)...");
    router.push("/");
  };
    ...
    <div className="min-h-screen bg-gray-50 flex font-sans selection:bg-blue-500/30">
      ...
      `}>
        ...
        <div className="p-8 flex items-center justify-between">
          ...
          <button onClick={() => setIsSidebarOpen(false)} className="md:hidden p-2 rounded-lg bg-gray-900 text-white hover:text-blue-400 transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <div className="p-6 bg-gray-900/50 border-t border-gray-800/60 mt-auto">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-gray-900/80 hover:bg-red-500/10 text-white hover:text-red-400 py-3 rounded-xl transition-all text-xs font-bold uppercase tracking-wider border border-gray-800 hover:border-red-500/20 group"
          >
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <header className="bg-white/80 backdrop-blur-md border-b border-gray-200/50 p-4 flex items-center justify-between md:hidden z-30 sticky top-0">
            ...
            <button onClick={() => setIsSidebarOpen(true)} className="p-2 bg-gray-100 rounded-xl text-gray-600 hover:bg-gray-200 transition-colors">
                <Menu size={20} />
            </button>
        </header>
